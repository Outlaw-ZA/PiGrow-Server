...
- `AutomationRule` — explicit per-device trigger rule.
  ...
  - `RuleCondition` enum (the six threshold conditions plus `ALWAYS_ON` / `ALWAYS_OFF` are accepted at the API layer):
    ...
    - `BELOW_MAX` — fires when the latest value falls below `*Max`. Common use: turn OFF a device an `ABOVE_MAX` rule switched ON (recovery / hysteresis-off). **Requires a non-null `watchedSensorType`.**
    - `ABOVE_MIN` — fires when the latest value rises above `*Min`. Common use: turn OFF a device a `BELOW_MIN` rule switched ON (recovery / hysteresis-off). **Requires a non-null `watchedSensorType`.**
    - `ABOVE_TARGET` — fires when the latest value exceeds the active phase's environment `*Target`. Tighter target-based control. **Requires a non-null `watchedSensorType`.**
    - `BELOW_TARGET` — fires when the latest value falls below `*Target`. Tighter target-based control. **Requires a non-null `watchedSensorType`.**
...
- **Automation engine** (`src/automation/evaluator.ts`): every persisted telemetry row is checked against enabled threshold rules (`ABOVE_MAX` / `BELOW_MIN` / `ABOVE_MIN` / `BELOW_MAX` / `ABOVE_TARGET` / `BELOW_TARGET`) whose `watchedSensorType` matches the reading. Hysteresis is enforced by reading the latest `DeviceStateLog` row for the device — the engine never issues a command that matches the device's already-confirmed state. The query also filters out `device.type = LIGHT` as a defensive measure against any pre-existing rule rows. **Per-device suppression:** an enabled `ALWAYS_ON` / `ALWAYS_OFF` rule covering `(device, scope, period)` skips evaluation of any threshold rules for that same `(device, scope, period)`. The pin itself is enforced by the scheduler, not the evaluator.
